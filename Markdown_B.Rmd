...
As a comparison, this is the result of a KS test on a graph generated with $\gamma$ = 1 (this means that every new link is added uniformly at random to already existing node).   

```{r}
set.seed(1765820)
graph_gamma = graph_generator(200, 1)
...
It is clear from the following graph and supported by the p-value KS-test on a power law fit, the generated graph does not follow a power law distribution.  
```{r}
cm_em_gamma_df = as.data.frame(cumulative_empirical(graph_gamma))
p5 <- ggplot(cm_em_gamma_df, aes(x=log(V1), y=log(V3))) +
  geom_point(size=1, color="#0294A5")+
  geom_smooth(method=lm, se=FALSE, linetype="dashed", color="#C1403D")+ 
